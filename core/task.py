...
{{
  ...
  "tasks": [
    ...
    def rebuild_from_skill(self, user_request: str, skill: dict) -> "TaskGraph":
        """
        从技能库的 graph_json 直接重建 TaskGraph，跳过 LLM 调用。
        把任务指令里的项目名/关键词替换为当前请求的关键词。
        """
        graph_json   = skill["graph_json"]
        old_project  = graph_json.get("project", "project")

        # 生成新项目名（从请求里提取，不调 LLM）
        import re as _re
        ...
        print(f"⚡ [Planner] 技能库命中，直接复用任务图（省略 LLM 规划）"
              f" | 置信度 {skill.get('_confidence', '?')}")
