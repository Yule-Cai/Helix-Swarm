...
class WorkflowStep(BaseModel):
    ...
    parallel_group: Optional[int] = Field(None, description="并行组ID（相同组的步骤可并行执行）")
...
class WorkflowEngine:
        ...
        while True:
            ...
            if not executable_steps:
                ...
                if not pending_steps:
                    break
                # 如果有 pending 步骤但无法执行，说明有依赖问题
                self._logger.error("Workflow has pending steps that cannot be executed")
                break

            # 将步骤按并行组分组
            parallel_groups = self._group_steps_for_parallel(executable_steps, workflow.max_concurrency)

            # 并发执行每组步骤
            for group in parallel_groups:
                if len(group) == 1:
                    # 单个步骤，直接执行
                    await self._execute_step(workflow, group[0])
                else:
                    # 多个步骤，并发执行
                    self._logger.info(f"Executing {len(group)} steps in parallel")
                    tasks = [
    ...
    def _group_steps_for_parallel(
        ...
        Returns:
            List[List[WorkflowStep]]: 步骤分组列表
        """
        # 按 parallel_group 分组
        groups: Dict[Optional[int], List[WorkflowStep]] = {}
        for step in steps:
        ...
        # 先执行有明确并行组的步骤
        for group_id, group_steps in sorted(groups.items(), key=lambda x: x[0] if x[0] is not None else float('inf')):
            if group_id is not None:
                # 按 max_concurrency 分割
                for i in range(0, len(group_steps), max_concurrency):
                    execution_plan.append(group_steps[i:i + max_concurrency])

        # 再执行没有并行组的步骤（每个单独执行）
        no_group_steps = groups.get(None, [])
        for step in no_group_steps:
